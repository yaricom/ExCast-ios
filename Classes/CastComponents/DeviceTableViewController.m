...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (_delegate.deviceManager.applicationConnectionState != GCKConnectionStateConnected) {
        self.title = NSLocalizedString(@"Connect to device", nil);
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    UIImage *playImage = paused ? [UIImage imageNamed:@"media_play"] : [UIImage imageNamed:@"media_pause"];
    ...
    if (_delegate.mediaInformation.metadata.images.count > 0) {
        ...
        dispatch_async(queue, ^{
            ...
            UIImage *image = [UIImage imageWithData:[SimpleImageFetcher getDataFromImageURL:mediaImage.URL]];
