...
- (void)showToolbar:(BOOL)show {
    BOOL hidden = !show;
    self.cc.hidden = hidden;
    self.currTime.hidden = hidden;
    self.totalTime.hidden = hidden;
    self.nextButton.hidden = hidden;
    self.previousButton.hidden = hidden;
    self.volumeButton.hidden = hidden;
...
- (IBAction)onSliderValueChanged:(id)sender {
    ...
    if (_castDeviceController.streamDuration > 0) {
        self.currTime.text = [self getFormattedTime:(pctThrough * _castDeviceController.streamDuration)];
    }
}
